...
server:
	docker run -d --rm --name apiserver --network unir-test --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
test-api:
	docker networkkk create calc-test-api || true
